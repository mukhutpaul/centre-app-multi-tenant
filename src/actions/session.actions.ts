...
export interface SessionModuleFormateursData {
  sessionId: string;
  moduleId: string;
  formateurIds?: string[];
}
...
function validateSessionData(data: SessionFormData) {
  ...
  if (data.capacite !== undefined && data.capacite !== null) {
    const capacite = Number(data.capacite);

    if (!Number.isInteger(capacite) || capacite <= 0) {
      errors.capacite = "La capacité doit être un nombre entier supérieur à 0.";
    }
  }
  ...
  if (data.statut && !Object.values(StatutSession).includes(data.statut)) {
      ...
      formation: {
        ...
        dureeHeures:
          session.formation.dureeHeures !== null
            ? Number(session.formation.dureeHeures.toString())
            : null,
      },
        ...
        module: {
          ...
          dureeHeures:
            moduleSession.module.dureeHeures !== null
              ? Number(moduleSession.module.dureeHeures.toString())
              : null,

          coefficient:
            moduleSession.module.coefficient !== null
              ? Number(moduleSession.module.coefficient.toString())
              : null,
        },
      })),
    };
...
export async function createSession(data: SessionFormData) {
  try {
    ...
    if (!canManageSessions(user.role)) {
      return {
        success: false,
        error: "Vous n'avez pas l'autorisation de créer une session.",
      };
    }
    ...
    if (!formation) {
      return {
        success: false,
        error: "Formation introuvable ou non accessible.",
      };
    }
    ...
    const existing = await prisma.sessionFormation.findFirst({
      where: {
        centreId: user.centreId,
        code,
      },
    ...
    if (existing) {
      return {
        success: false,
        error: `Le code de session "${code}" est déjà utilisé.`,
      };
    }
    ...
    const session = await prisma.sessionFormation.create({
      data: {
        ...
        capacite:
          data.capacite === null || data.capacite === undefined
            ? null
            : Number(data.capacite),

        statut: data.statut || StatutSession.PLANIFIEE,

        ouvertureInscriptions: data.ouvertureInscriptions
          ? new Date(data.ouvertureInscriptions)
          : null,

        fermetureInscriptions: data.fermetureInscriptions
          ? new Date(data.fermetureInscriptions)
          : null,
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2002") {
        return {
          success: false,
          error: "Une session avec ce code existe déjà dans ce centre.",
        };
      }
    }
...
export async function updateSession(id: string, data: SessionFormData) {
  try {
    ...
    if (!canManageSessions(user.role)) {
      return {
        success: false,
        error: "Vous n'avez pas l'autorisation de modifier une session.",
      };
    }
    ...
    if (data.capacite !== null && data.capacite !== undefined) {
      const nombreInscriptions = await prisma.inscription.count({
        where: {
          sessionId: id,
      ...
      if (Number(data.capacite) < nombreInscriptions) {
        return {
          success: false,
          error:
    ...
    const updated = await prisma.sessionFormation.update({
      where: {
        id,
      },
...
export async function updateSessionStatut(id: string, statut: StatutSession) {
  try {
    const authResult = await getAuthenticatedUser();
    ...
    if (!Object.values(StatutSession).includes(statut)) {
      return {
        success: false,
        error: "Statut de session invalide.",
      };
    }
    ...
    console.error("Erreur updateSessionStatut():", error);

    return {
      success: false,
      error: "Impossible de modifier le statut de la session.",
    };
  }
}
...
export async function deleteSession(id: string) {
  try {
    ...
    if (!canManageSessions(user.role)) {
      return {
        success: false,
        error: "Vous n'avez pas l'autorisation de supprimer une session.",
      };
    }
    ...
    if (modulesCount > 0) {
      dependencies.push(`${modulesCount} module(s)`);
    }

    if (formateursCount > 0) {
      dependencies.push(`${formateursCount} formateur(s)`);
    }

    if (inscriptionsCount > 0) {
      dependencies.push(`${inscriptionsCount} inscription(s)`);
    }

    if (planningCount > 0) {
      dependencies.push(`${planningCount} élément(s) de planning`);
    }

    if (jurysCount > 0) {
      dependencies.push(`${jurysCount} jury(s)`);
    }

    if (resultatsCount > 0) {
      dependencies.push(`${resultatsCount} résultat(s)`);
    }
...
export async function getFormationsForSession() {
  try {
    ...
    const formationsSerialisees = formations.map((formation) => ({
      id: formation.id,
      code: formation.code,
      nom: formation.nom,
      type: formation.type,
      ...
      dureeHeures:
        formation.dureeHeures !== null
          ? Number(formation.dureeHeures.toString())
          : null,
    ...
    console.error("Erreur getFormationsForSession():", error);
...
export async function getSessionModules(sessionId: string) {
  try {
    const authResult = await getAuthenticatedUser();
    ...
    const modulesSerialises = modules.map((module) => {
      const moduleSession = module.modulesSessions[0] ?? null;
      ...
      return {
        ...
        formationPosition: module.position,

        dureeHeures:
          module.dureeHeures !== null ? Number(module.dureeHeures) : null,

        coefficient:
          module.coefficient !== null ? Number(module.coefficient) : null,

        estDansSession: moduleSession !== null,

        moduleSessionId: moduleSession?.id ?? null,

        positionSession: moduleSession?.position ?? null,

        dateDebut: moduleSession?.dateDebut ?? null,

        dateFin: moduleSession?.dateFin ?? null,

        nombreFormateurs: moduleSession?._count.formateurs ?? 0,

        nombreEvaluations: moduleSession?._count.evaluations ?? 0,

        formateurs:
          moduleSession?.formateurs.map(({ formateur }) => ({
            id: formateur.id,
    ...
    return {
      success: true,
      modules: modulesSerialises,
    };
  } catch (error) {
    console.error("Erreur getSessionModules :", error);

    return {
      success: false,
      error: "Impossible de récupérer les modules de la session.",
      modules: [],
    };
  }
}
...
// ============================================================
// AJOUTER UN MODULE À UNE SESSION
// AVEC AFFECTATION DE FORMATEURS
// ============================================================
  ...
  try {
    ...
    if (!canManageSessions(user.role)) {
      return {
        success: false,
        error:
          "Vous n'avez pas l'autorisation de modifier les modules de cette session.",
      };
    }
    ...
    if (!sessionId?.trim() || !moduleId?.trim()) {
      return {
        success: false,
        error: "Les informations du module sont invalides.",
      };
    }

    // --------------------------------------------------------
    // NETTOYER LES IDS DES FORMATEURS
    // --------------------------------------------------------

    const cleanFormateurIds = [
      ...new Set(
        (formateurIds ?? [])
          .filter(
            (id): id is string =>
    ...
    const module = await prisma.moduleFormation.findFirst({
      where: {
        id: moduleId,
        formationId: session.formationId,
      },
    ...
    let formateursValides: {
      id: string;
      prenom: string;
      nom: string;
    }[] = [];

    if (cleanFormateurIds.length > 0) {
      formateursValides = await prisma.formateur.findMany({
        where: {
          id: {
            in: cleanFormateurIds,
          },
      ...
      if (
        formateursValides.length !==
        cleanFormateurIds.length
      ) {
        const idsTrouves = new Set(
          formateursValides.map((formateur) => formateur.id),
        );

        const idsInvalides = cleanFormateurIds.filter(
          (id) => !idsTrouves.has(id),
        );

        console.error(
          ">>> Formateurs invalides :",
          idsInvalides,
        );
        ...
        return {
          success: false,
          error:
            "Un ou plusieurs formateurs sélectionnés sont invalides, inactifs ou n'appartiennent pas à votre centre.",
        };
      }
    }
    ...
    //
    // IMPORTANT :
    // On fait le calcul de position DANS la transaction.
    //
    // Et on utilise create() au lieu de createMany().
    // --------------------------------------------------------

    const result = await prisma.$transaction(async (tx) => {
      // ------------------------------------------------------
      // VÉRIFIER UNE NOUVELLE FOIS LE DOUBLON
      // ------------------------------------------------------

      const existingModuleSession =
        await tx.moduleSession.findUnique({
          where: {
            sessionId_moduleId: {
              sessionId,
              moduleId,
      ...
      if (existingModuleSession) {
        throw new Error(
          "Ce module est déjà associé à cette session.",
        );
      }
      ...
      const lastModuleSession =
        await tx.moduleSession.findFirst({
          where: {
            sessionId,
          },
      ...
      const nextPosition =
        (lastModuleSession?.position ?? 0) + 1;

      // ------------------------------------------------------
      // CRÉER MODULE SESSION
      // ------------------------------------------------------

      const createdModuleSession =
        await tx.moduleSession.create({
          data: {
            sessionId,
            moduleId,
      ...
      // ------------------------------------------------------
      //
      // On crée chaque relation séparément.
      //
      // Tout est dans la même transaction.
      // Si une affectation échoue :
      //
      // ModuleSession
      ...
      for (const formateurId of cleanFormateurIds) {
        await tx.formateurModuleSession.create({
          data: {
            moduleSessionId: createdModuleSession.id,
            formateurId,
          },
        });
      }
      ...
      return {
        ...createdModuleSession,
        nombreFormateurs: cleanFormateurIds.length,
      };
    });
    ...
    return {
      ...
      message:
        cleanFormateurIds.length > 0
          ? `Le module "${module.nom}" a été ajouté avec ${cleanFormateurIds.length} formateur(s).`
          : `Le module "${module.nom}" a été ajouté à la session.`,
    ...
    if (
      ...
      if (error.code === "P2002") {
        return {
          success: false,
          error:
            "Ce module ou ce formateur est déjà associé à cette session.",
        };
      }
      ...
      if (error.code === "P2003") {
        return {
          success: false,
          error:
            "Impossible d'associer le formateur à ce module. Vérifiez les relations entre le module, la session et le formateur.",
        };
      }
    }
    ...
    return {
      success: false,
      error:
        "Une erreur inattendue est survenue lors de l'ajout du module.",
    };
  }
}
...
export async function removeModuleFromSession(
  ...
  try {
    ...
    if (!sessionId || !moduleId) {
      return {
        success: false,
        error: "Session ou module invalide.",
      };
    }
    ...
    if (!moduleSession) {
      return {
        success: false,
        error: "Ce module n'est pas présent dans cette session.",
      };
    }
    ...
    if (
      ...
      if (moduleSession._count.formateurs > 0) {
        raisons.push(`${moduleSession._count.formateurs} formateur(s)`);
      }

      if (moduleSession._count.evaluations > 0) {
        raisons.push(`${moduleSession._count.evaluations} évaluation(s)`);
      }
      ...
      return {
        ...
        error:
          ...
          `Il est déjà utilisé par ${raisons.join(" et ")}.`,
      };
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const remaining = await tx.moduleSession.findMany({
        where: {
          sessionId,
        },
    ...
    return {
      success: true,
      message: `Le module "${moduleSession.module.nom}" a été retiré de la session.`,
    };
  } catch (error) {
    console.error("Erreur removeModuleFromSession():", error);

    return {
      success: false,
      error: "Impossible de retirer le module de la session.",
    };
  }
}
...
export async function updateSessionModulePosition(
  ...
  try {
    ...
    if (!canManageSessions(user.role)) {
      return {
        success: false,
        error: "Vous n'avez pas l'autorisation de réordonner les modules.",
      };
    }
    ...
    if (!Number.isInteger(position) || position < 1) {
      return {
        success: false,
        error: "La position doit être un entier supérieur ou égal à 1.",
      };
    }
    ...
    const current = await prisma.moduleSession.findFirst({
      where: {
        id: moduleSessionId,
        sessionId,
      },
    ...
    if (!current) {
      return {
        success: false,
        error: "Module introuvable dans cette session.",
      };
    }
    ...
    const modules = await prisma.moduleSession.findMany({
      where: {
        sessionId,
      },
    ...
    if (position > total) {
      return {
        success: false,
        error: `La position maximale est ${total}.`,
      };
    }
    ...
    if (position === current.position) {
      return {
        success: true,
        message: "La position du module est déjà correcte.",
      };
    }
    ...
    const reordered = modules.filter((item) => item.id !== moduleSessionId);

    reordered.splice(position - 1, 0, {
      id: moduleSessionId,
      position: 0,
    });
    ...
    console.error("Erreur updateSessionModulePosition():", error);

    return {
      success: false,
      error: "Impossible de réordonner les modules.",
    };
  }
}
...
export async function moveSessionModuleUp(
  ...
  try {
    ...
    if (!canManageSessions(authResult.user.role)) {
      return {
        success: false,
        error: "Vous n'avez pas l'autorisation de réordonner les modules.",
      };
    ...
    const current = await prisma.moduleSession.findFirst({
      where: {
        ...
        session: {
          centreId: authResult.user.centreId,
        },
      },
    ...
    if (current.position <= 1) {
      return {
        success: false,
        error: "Le module est déjà en première position.",
      };
    }
    ...
    console.error("Erreur moveSessionModuleUp():", error);

    return {
      success: false,
      error: "Impossible de déplacer le module.",
    };
  }
}
...
export async function moveSessionModuleDown(
  ...
  try {
    ...
    const total = await prisma.moduleSession.count({
      where: {
        sessionId,
      },
    });
    ...
    if (current.position >= total) {
      return {
        success: false,
        error: "Le module est déjà en dernière position.",
      };
    }
    ...
    console.error("Erreur moveSessionModuleDown():", error);
...
export async function getFormateursForSession(sessionId: string) {
  try {
    const authResult = await getAuthenticatedUser();
    ...
    if (!canManageSessions(user.role)) {
      return {
        success: false,
        error: "Vous n'avez pas l'autorisation de gérer les formateurs.",
        formateurs: [],
      };
    }
    ...
    console.error("Erreur getFormateursForSession :", error);

    return {
      success: false,
      error: "Impossible de récupérer les formateurs.",
      formateurs: [],
    };
  }
}
